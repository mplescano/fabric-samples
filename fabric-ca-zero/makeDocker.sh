...
function writeSetupFabric {
   echo "  setup:
    container_name: setup-mplescano-com
    hostname: setup.mplescano.com
...
function writeRunFabric {
   ...
   echo "  run:
    container_name: run-mplescano-com
    hostname: run.mplescano.com
    image: hyperledger/fabric-ca-tools
    environment:
      - GOPATH=/opt/gopath
    command: /bin/bash -c 'sleep 3;/scripts/run-fabric-join.sh 2>&1 | tee /$RUN_LOGFILE; sleep 99999'
    volumes:
      - ./scripts:/scripts
      - ./$DATA:/$DATA
...
function writeSetupSdks {
   # Set samples directory relative to this script
   SAMPLES_DIR=$(dirname $(cd ${SDIR} && pwd))
   # Set fabric directory relative to GOPATH
   FABRIC_DIR=${GOPATH}/src/github.com/hyperledger/fabric
   SDKS_LOGFILE=${LOGDIR}/setup-sdks-phase2.log
   echo "  sdks:
    container_name: sdks-mplescano-com
    hostname: sdks.mplescano.com
    build:
      context: .
      dockerfile: ./dockerfiles/DockerfileSetupSdksPhase1"
   ...
   if [[ ! -z "${no_proxy}" ]]; then
    ...
    echo "
    image: mplescano/hyperledger-setup-sdks
    environment:
      - GOPATH=/opt/gopath"
   if [[ ! -z "${http_proxy}" ]]; then
     echo "      - http_proxy=$http_proxy"
...
function writeRootCA {
   echo "  $ROOT_CA_HOST:
    ...
    environment:
      ...
      - FABRIC_CA_SERVER_CSR_CA_PATHLENGTH=$(if [ $USE_INTERMEDIATE_CA == true ]; then echo '1'; else echo '0'; fi)
      - FABRIC_CA_SERVER_CSR_HOSTS=$ROOT_CA_HOST
      - FABRIC_CA_SERVER_DEBUG=$DEBUG_MODE
...
function writeOrderer {
   ...
   echo "  $ORDERER_HOST:
    ...
    environment:
      ...
      - ORDERER_GENERAL_TLS_CLIENTAUTHREQUIRED=$CLIENT_AUTH_REQUIRED
      - ORDERER_GENERAL_TLS_CLIENTROOTCAS=[$CA_CHAINFILE]
      - ORDERER_GENERAL_LOGLEVEL=$DEBUG_LOG_LEVEL
      - ORDERER_DEBUG_BROADCASTTRACEDIR=$LOGDIR
      - ORG=$ORG
      - ORG_ADMIN_CERT=$ORG_ADMIN_CERT
    command: /bin/bash -c '/scripts/start-orderer.sh 2>&1 | tee /$ORDERER_LOGFILE; sleep 99999'
    volumes:
      - ./scripts:/scripts
      - ./$DATA:/$DATA
    networks:
...
function writePeer {
   ...
   echo "  $PEER_HOST:
    ...
    environment:
      ...
      - CORE_LOGGING_LEVEL=$DEBUG_LOG_LEVEL
      ...
      - CORE_PEER_TLS_CLIENTAUTHREQUIRED=$CLIENT_AUTH_REQUIRED
